function New-DCNMVlanInterface           {
    <#
 .SYNOPSIS
Create a new SVI
 .DESCRIPTION
This cmdlet will invoke a REST POST against the DCNM API Global Interface
 .EXAMPLE
New-DCNMVlanInterface -Fabric site3 -SwitchName SPINE-4 -Vlan 2021 -Description SomeNetwork2021 -MTU 1500 -Prefix 30.30.30.30/24 -Tag 76894 -VRF myvrf
...
# Set a tag of 76894, and place in the VRF "myvrf"
 .EXAMPLE
New-DCNMVlanInterface -Fabric site3 -SwitchName LEAF-6 -Vlan 2222 -Prefix 22.22.22.1/24 -CliFreeform @"
>>  bandwidth 101010
>>  delay 40000
>> "@ -Enabled false
...
# Use the freeform template to create an SVI for VLAN 2222, with an IP address 22.22.22.1 255.255.255.0, and CLI Freeform containing custom b/w & delay values, and create in the SHUTDOWN state
 .EXAMPLE
(Import-Csv .\Book2.csv)[2..4] | New-DCNMVlanInterface

# Use a CSV file with headers corresponding to parameters to bulk-import interface configurations
 .EXAMPLE
New-DCNMVlanInterface -Fabric site3 -SwitchName LEAF-6 -Vlan 2222 -Prefix 192.168.2.253/24 -HsrpGroup 2 -HsrpVIP 192.168.2.1

# Create an SVI with HSRP configuration
 .PARAMETER Fabric
Name of the fabric
 .PARAMETER SwitchName
Name of the switch
 .PARAMETER Vlan
VLAN for which the SVI will be created for
